...
function DoLua(L: Plua_state): integer; cdecl;//IceMaple添加
function ExecLuaFunc(L: Plua_state): integer; cdecl;//IceMaple添加
...
function PutItemIntro(L: Plua_state): integer; cdecl; //IceMaple添加

function GetExtraInt(L: Plua_state): integer; cdecl; //IceMaple添加
function PutExtraInt(L: Plua_state): integer; cdecl; //IceMaple添加
function GetExtraStr(L: Plua_state): integer; cdecl; //IceMaple添加
function PutExtraStr(L: Plua_state): integer; cdecl; //IceMaple添加
function GetNPCPro(L: Plua_state): integer; cdecl;//IceMaple添加
function PutNPCPro(L: Plua_state): integer; cdecl;//IceMaple添加
function LuaCanWalk(L: Plua_state): integer; cdecl;//IceMaple添加
function LuaNewShop(L: Plua_state): integer; cdecl;//IceMaple添加
...
function LuaUpdateShopList(L: Plua_state): integer; cdecl;
...
begin
  ...
  lua_register(Lua_script, 'dolua', DoLua);//IceMaple添加
  lua_register(Lua_script, 'execluafunc', ExecLuaFunc);//IceMaple添加
  ...
  lua_register(Lua_script, 'putitemintro', PutItemIntro); //IceMaple添加

  lua_register(Lua_script, 'getextraint', GetExtraInt); //IceMaple添加
  lua_register(Lua_script, 'putextraint', PutExtraInt); //IceMaple添加
  lua_register(Lua_script, 'getextrastr', GetExtraStr); //IceMaple添加
  lua_register(Lua_script, 'putextrastr', PutExtraStr); //IceMaple添加
  lua_register(Lua_script, 'getnpcpro', GetNPCpro); //IceMaple添加
  lua_register(Lua_script, 'putnpcpro', PutNPCpro); //IceMaple添加
  lua_register(Lua_script, 'canwalk', LuaCanWalk); //IceMaple添加
  lua_register(Lua_script, 'newshop', LuaNewShop); //IceMaple添加
  lua_register(Lua_script, 'updateshoplist', LuaUpdateShopList); //IceMaple添加
...
function ExecLuaFunc(L: Plua_state): integer; cdecl;
var
  Script, filename, funcwithargin: string;
  n, h, len: integer;
begin
  n:=lua_gettop(L);
  filename:=ExtractFilePath(ParamStr(0)) + 'script\' + lua_tostring(L,-n)+'.lua';
  funcwithargin:=lua_tostring(L,-n+1)+';';
  Script := '';
  ...
  begin
    ...
    if Result = 0 then
    begin
      if funcwithargin <> '' then
      begin
        lua_pcall(Lua_script, 0, 1, 0);
        Result := luaL_dostring(Lua_script,pchar(funcwithargin));
        if Result <> 0 then
        begin
...
//通过传递参数的方法，lua调用pascal，再调用lua（但还有问题）
{function ExecLuaFunc(L: Plua_state): integer; cdecl;
var
  Script, filename, functionname: string;
  n, h, len, i,count: integer;
begin
  n:=lua_gettop(L);
  filename:=ExtractFilePath(ParamStr(0)) + 'script\' + lua_tostring(L,-n)+'.lua';
  functionname=lua_tostring(L,-n+1);
  Script := '';
  if FileExists(filename) then
  begin
    h := FileOpen(filename, fmopenread);
    ...
    //{$ENDIF}
    {Result := lual_loadbuffer(Lua_script, @script[1], length(script), 'code');
    writeln('load successfully');
    if Result = 0 then
    begin
      if functionname <> '' then
      begin
        lua_getglobal(Lua_script, pchar(functionname));
        writeln('L1=',lua_tointeger(L,-n));
        writeln('L2=',lua_tointeger(L,-n+1));
        writeln('function');
        if n>2 then
        begin
           count:=1;
           ...
           begin
               writeln('count= ',count);
               writeln('lua_isnumber=',lua_isnumber(L,i));
               writeln('lua_isstring=',lua_isstring(L,i));
               writeln('lua_isboolean=',lua_isboolean(L,i));
               writeln('L1=',lua_tointeger(L,i));
               if lua_isnumber(L,i) then
               begin
                  lua_tointeger(L,i);
                  writeln(lua_tointeger(L,i));
                  lua_pushinteger(Lua_script, lua_tointeger(L,i));
               end
               else if lua_isstring(L,i) then
               begin
                       writeln(lua_tostring(L,i));
                       lua_pushstring(Lua_script, lua_tostring(L,i));
               end
               else if lua_isboolean(L,i) then
               begin
                       writeln(lua_tostring(L,i));
                       lua_pushboolean(Lua_script, lua_toboolean(L,i));
               end;
               count:=count+1;
           end;
        end;
        Result := lua_pcall(Lua_script, n-2, 1, 0);
...
//IceMaple添加
function DoLua(L: Plua_state): integer; cdecl;
var
  filename: string;
begin
  writeln('AppPath=',AppPath);
  filename:=AppPath + 'script/' + lua_tostring(L, -2);
  ExecScript(filename,lua_tostring(L, -1));
  Result := 1;
end;
...
begin
  ...
  case dismode of
    ...
    begin
      ...
      heady := 85;
      diagx := 100;
      diagy := 30;                  Move(Rrole[i].Name[0], Name[0], len);
    end;
    1:
    begin
      headx := 546;
...
//读整数全局变量信息（By Icemaple）
function GetExtraInt(L: Plua_state): integer; cdecl;
begin
  lua_pushinteger(L, Eint[lua_tointeger(L, -1)]);
  Result := 1;
end;

//写整数全局变量信息（By Icemaple）
function PutExtraInt(L: Plua_state): integer; cdecl;
begin
  Eint[lua_tointeger(L, -1)] := lua_tointeger(L, -2);
  Result := 0;
end;


//读字符串全局变量信息（By Icemaple）
function GetExtraStr(L: Plua_state): integer; cdecl;
var
  n, strnum, i, maxlen: integer;
  s: string;
  str: WideString;
  p: pWideChar;
begin
  n:=lua_tointeger(L, -1);
  strnum:=lua_tointeger(L, -2);
  if (n = 0) then
  begin
  maxlen:=10;
  p:= @Ename[strnum].s[0];
  ...
  //s:=WideCharToString(PWideChar(str));
  s:=WideCharToString(p);
  lua_pushstring(L, s);
  Result := 1;
end;

//写字符串全局变量信息（By Icemaple）
function PutExtraStr(L: Plua_state): integer; cdecl;
var
  n, strnum, i, len, maxlen: integer;
  str: WideString;
  p: pWideChar;
begin
  n:=lua_tointeger(L, -1);
  ...
  begin
  maxlen:=10;
  FillChar(Ename[strnum].s, sizeof(Ename[strnum].s), 0);
  p:= @Ename[strnum].s[0];
  end
  else
  begin
  maxlen:=15;
  FillChar(Estring[strnum].s, sizeof(Estring[strnum].s), 0);
  p:= @Estring[strnum].s[0];
  end;
  if (len > maxlen) then
  begin
    writeln('String length is too long!');
  end
  else
  begin
    for i := 1 to len do
...
function GetNPCPro(L: Plua_state): integer; cdecl;
begin
  lua_pushinteger(L, ENPC[lua_tointeger(L, -2)].Data[lua_tointeger(L, -1)]);
  Result := 1;
...
function PutNPCPro(L: Plua_state): integer; cdecl;
begin
  ENPC[lua_tointeger(L, -2)].Data[lua_tointeger(L, -1)] := lua_tointeger(L, -3);
  Result := 0;
end;

//判断是否可以行走（By Icemaple）
function LuaCanWalk(L: Plua_state): integer; cdecl;
var
  notentrance, walkres, res, ifentrance: boolean;
  x,y,i,j: integer;
begin
  x:= lua_tointeger(L, -1);
  y:= lua_tointeger(L, -2);
  ...
  begin
    for j:=-1 to 1 do
    begin
      ifentrance:=ifentrance or (Entrance[x+i,y+j] > -1);
    end;
  end;
  notentrance:= not ifentrance;
  walkres:= CanWalk(x,y);
  res:=notentrance and walkres;
...
function LuaNewShop(L: Plua_state): integer; cdecl;
begin
...
function LuaUpdateShopList(L: Plua_state): integer; cdecl;
begin
    UpdateShopList(lua_tointeger(L, -2),lua_tointeger(L, -1));
    Result := 0;
end;
