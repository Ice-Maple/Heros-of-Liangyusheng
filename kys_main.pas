...
function LoadR(num: integer): boolean;
var
  zfilename, filenamer, filenames, filenamed, filenamee,s: string;
  str, p, p1, p2: pchar;
  key1, key2: pbyte;
...
begin
  ...
  begin
    if FileExists(AppPath + 'save/' + filenamer) and FileExists(AppPath + 'save/' + filenames) and
      FileExists(AppPath + 'save/' + filenamed) and FileExists(AppPath + 'save/' + filenamee) then
    begin
      ...
      FileClose(grp);
      grp := FileOpen(AppPath + 'save/' + filenamee, fmopenread);
      FileRead(grp, p2^, lenE);
      FileClose(grp);
    end
    else
    begin
  ...
  begin
    ...
    BufferRead(p2, @Eint[0], EnameOffset);
    BufferRead(p2, @Ename[0], EstringOffest-EnameOffset);
    BufferRead(p2, @Estring[0], ENPCOffest-EstringOffest);
    BufferRead(p2, @ENPC[0], LenE-ENPCOffest);
...
begin
  ...
  p0 := StrAlloc(LenE + 4);
  p2 := p0;
  BufferWrite(p2, @Eint[0], EnameOffset);
  BufferWrite(p2, @Ename[0], EstringOffest-EnameOffset);
  BufferWrite(p2, @Estring[0], ENPCOffest-EstringOffest);
  BufferWrite(p2, @ENPC[0], LenE-ENPCOffest);
  ...
  if num = 0 then
    filenamed := 'alldef.grp';
  filenamee := 'e' + s + '.grp';//extra文件，IceMaple添加
  if num = 0 then
    filenamee := 'extra.grp';
  ...
  begin
    ...
    grp := filecreate(AppPath + 'save/' + filenamee, fmopenreadwrite);
    if grp < 0 then
      Result := False
    else
    begin
      FileSeek(grp, 0, 0);
      FileWrite(grp, p0^, LenE);
      FileClose(grp);
    end;
    //IceMaple添加，extra文件存档
  end
  else
  begin
...
var
  ...
  pos: TPosition;
  trigger_rand:longint;
  trigger_count:integer;
  trigger_str: WideString;
begin
  if where >= 3 then
    exit;
  ...
  begin
    ...
    begin
       ...
       if (trigger_count mod 100 =0) then
       begin
          trigger_count:=0;
          trigger_rand :=random(100);
          if trigger_rand>70 then
          begin
             ExecScript(AppPath + 'script/MMap_Trigger.lua', trigger_str);
             writeln('MMap_Trigger');
          end;
      end;
    end;
