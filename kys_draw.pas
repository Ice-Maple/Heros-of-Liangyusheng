...
procedure DrawScence;
var
  x, y, xpoint, ypoint, axp, ayp, i, i1, i2, num, sum, widthregion, sumregion, cx1, cy1, filenum : integer;
  dest: TSDL_Rect;
  word: WideString;
  pos: Tposition;
  BigPicFlag: boolean;
begin
  ...
  //IceMaple修改，可读入大图，大图为4130+当前场景号
  //当读入大图时，不画Sdata中的地面层
  filenum:= CurScence+4130;
  if FileExists(AppPath + 'resource/smap/' + IntToStr(filenum) + '.png') then
  begin
     BigPicFlag:=true;
     Pos := GetPositionOnScreen(31, 31, Cx1, Cy1);
     DrawSPic(filenum, pos.x, pos.y);
  end;
  ...
  for sum := -sumregion to sumregion + 15 do
    ...
    begin
      ...
      begin
        ...
        if (not BigPicFlag) and (SData[CurScence, 4, i1, i2] > 0) then
        begin
  ...
  //IceMaple修改添加，可读入大图遮罩，为4530+当前场景号（大图序号+400）
  filenum:= filenum+400;
  if BigPicFlag and FileExists(AppPath + 'resource/smap/' + IntToStr(filenum) + '.png') then
  begin
     Pos := GetPositionOnScreen(31, 31, Cx1, Cy1);
     DrawSPic(filenum, pos.x, pos.y);
  end;
...
begin
  ...
  //绝情谷, 明教地道效果比较奇怪, 屏蔽
  if (EXPAND_GROUND <> 0) and ((MODVersion <> 13) and ((CurScence <> 81) and (CurScence <> 72))) then
  begin
