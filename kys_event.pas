...
procedure UpdateShopList(shop_num, shoptype: integer);
function CreatItemList(Pblist,itemlevelconst: TIntArray): TIntArray;

procedure ShowMap;
function EnterNumber(MinValue, MaxValue, x, y: integer; Default: integer = 0): smallint;
procedure InitialItemLevelList;
function RandomN(n,amount:integer): TIntArray;
function RandomPb(n:integer;Pblist: TIntArray): integer;
...
procedure InitialItemLevelList;
var
  inum,jnum,itemlevel,canbesold:integer;
  count:array[1..10] of smallint;
begin
  for inum:=1 to 10 do
  begin
    count[inum]:=0;
  end;
  for jnum:=0 to MAX_ITEM_AMOUNT-1 do
  begin
      itemlevel:=Ritem[jnum].UnUse[0];
      canbesold:=Ritem[jnum].UnUse[1];
      if (itemlevel>0) and (canbesold=1) and (Ritem[jnum].User<0) then
      begin
         count[itemlevel]:=count[itemlevel]+1;
         ItemLevelList[itemlevel,count[itemlevel]]:=jnum;
      end;
  end;
  for inum:=1 to 10 do
  begin
    ItemLevelList[inum,0]:=count[inum];
  end;
  {for inum:=1 to 10 do
  begin
    writeln('i=',inum);
    for jnum:=0 to 100 do
    begin
      if ItemLevelList[inum,jnum]>0 then
       writeln(ItemLevelList[inum,jnum]);
    end;
  end;}
end;
...
function RandomN(n,amount:integer): TIntArray;
var
  inum,jnum,randx,len: integer;
  randlist, randmap:TIntArray;
begin
   setlength(randlist,amount);
   setlength(randmap,n);
   if n<amount then
   exit;
   for jnum:=0 to n-1 do
     randmap[jnum]:=jnum;
   len:=n-1;
   for inum:=0 to amount-1 do
   begin
     randomize;
     randx:=random(len);
     randlist[inum]:=randmap[randx];
     if randx<len then
     begin
       for jnum:=randx + 1 to len do
          randmap[jnum-1]:=randmap[jnum];
     end;
     len:=len-1;
   end;
   result:=randlist;
...
function RandomPb(n:integer;Pblist: TIntArray): integer;
var
  inum, randx,pbmax,randn: integer;
  Pbcount: TIntArray;
begin
   if n<>Length(Pblist) then
   begin
     writeln('Pblist size is wrong!');
     exit;
   end;
   pbmax:=100;
   setlength(Pbcount,n+1);
   Pbcount[0]:=0;
   for inum:=1 to n do
   begin
      Pbcount[inum]:=Pbcount[inum-1]+Pblist[inum-1];
   end;
   randx:=random(pbmax-1);
   for inum:=1 to n do
   begin
     if (randx>=Pbcount[inum-1]) and (randx<Pbcount[inum]) then
     begin
        randn:=inum-1;
        break;
     end;
...
//随机更新shop列表(IceMaple添加)
//shoptype取值：1：大地图普通；2：大地图稀有；3：内景随机普通；4：内景随机稀有；5：小城市；6中城市；7大城市
//一般shop_num：1~4和shoptype：1~4固定对应
procedure UpdateShopList(shop_num, shoptype: integer);
var
  AllItemList, Pblist, itemlevelconst, itempbnum, randlist, itemlist:array of integer;
  inum,jnum,tempb,count,itemid,len: integer;
  itemlevel:array[1..5] of integer;
begin
  InitialItemLevelList;
  case shoptype of
    1:
    begin
      len:=3;
      setlength(Pblist,len);
      setlength(itemlevelconst,len);
      Pblist[0]:=60;
      Pblist[1]:=30;
      Pblist[2]:=10;
      itemlevelconst[0]:=5;
   ...
   itemlist:=CreatItemList(Pblist,itemlevelconst);
   for inum:=0 to 4 do
   begin
     itemid:=itemlist[inum];
     Rshop[shop_num].Item[inum]:=itemid;
     Rshop[shop_num].Amount[inum]:=1;
     Rshop[shop_num].Price[inum]:=RItem[itemid].Price;
     if RItem[itemid].UnUse[2]>0 then
        Rshop[shop_num].Amount[inum]:=RItem[itemid].UnUse[2];
   end;
end;

function CreatItemList(Pblist,itemlevelconst: TIntArray): TIntArray;
var
  itempbnum, randlist, itemlist:array of integer;
  inum,jnum,itempb,count,itemid,len: integer;
begin
  setlength(itemlist,5);
  len:=Length(Pblist);
  setlength(itempbnum,len);
  for inum := 0 to len-1 do
      itempbnum[inum]:=0;
  for inum := 1 to 5 do
  begin
     itempb:=RandomPb(len,Pblist);
     for jnum:=0 to len-1 do
     begin
       if itempb=jnum then
       itempbnum[itempb]:=itempbnum[itempb]+1;
     end;
  end;
  count:=0;
  for inum:=0 to len-1 do
  begin
    if itempbnum[inum]>0 then
    begin
     if ItemLevelList[itemlevelconst[inum],0]>0 then
     begin
        if ItemLevelList[itemlevelconst[inum],0]<itempbnum[inum] then
        begin
           for jnum:=0 to itempbnum[inum]-1 do
           begin
               itemid:=ItemLevelList[itemlevelconst[inum],1];
               itemlist[count]:=itemid;
               count:=count+1;
           end;
        end
        else
        randlist:=RandomN(ItemLevelList[itemlevelconst[inum],0],itempbnum[inum]);
        for jnum:=0 to itempbnum[inum]-1 do
        begin
           itemid:=ItemLevelList[itemlevelconst[inum],randlist[jnum]+1];
           itemlist[count]:=itemid;
           count:=count+1;
        end;
     end;
